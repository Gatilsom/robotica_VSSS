...
def draw_field(screen, field_length=1.5, field_width=1.3, color=(255, 0, 0)):
    field_chamfer = field_length * 7 / 150
    field_center = field_length * 2 / 15

    side_lines_top = pg.draw.lines(screen, color, closed=False,
                                   points=[(25, 25 + length2pixel(field_width / 2 - field_width * 2 / 13)),
                                           ...
                                           (
                                               ...
                                               25 + length2pixel(field_width / 2 - field_width * 2 / 13))

                                           ], width=3)

    side_lines_bottom = pg.draw.lines(screen, color, closed=False,
                                      points=[
                                          (25, 25 + length2pixel(field_width / 2 + field_width * 2 / 13)),
    ...
    left_circle = pg.draw.arc(screen, color, (25 + length2pixel(field_length * 3 / 28) - length2pixel(10.09 / 100) / 2,
                                              ...
                                              length2pixel(field_width * 2 / 13)), np.deg2rad(-90), np.deg2rad(90))
    ...
    right_circle = pg.draw.arc(screen, color, (
        ...
        25 + length2pixel(field_width / 2) - length2pixel(20 / 100) / 2, length2pixel(field_length * 1009 / 15000),
        length2pixel(field_width * 2 / 13)), np.deg2rad(90), np.deg2rad(-90))
...
def draw_ball(screen, field_length=1.5, field_width=1.3, radius=42.7 / 2000, color="orange"):
    x_pos = randint(25, 25 + length2pixel(field_length))
    y_pos = randint(25, 25 + length2pixel(field_width))
